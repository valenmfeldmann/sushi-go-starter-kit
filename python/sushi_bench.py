...
# --- Configuration ---
WINDOWS_MODE = False  # Set to True for Windows, False for Linux
PYTHON_CMD = "python" if WINDOWS_MODE else "python3"

SERVER_HOST = "localhost"
SERVER_PORT = 7878
WEB_URL = "http://localhost:8080/api/games"
STRATEGIES = ["first_card_bot.py", "sushi_go_client.py", "sushi_go_client.py", "b_v_sushi_go_client_card_aware_TEST1.py"]
NUM_ROUNDS = 10
...
def print_leaderboard(win_matrix, strat_names):
    """Prints bots ordered by win count with the best at the top."""
    total_wins = np.sum(win_matrix, axis=1)
    leaderboard = sorted(zip(strat_names, total_wins), key=lambda x: x[1], reverse=True)

    print("\n" + "=" * 40)
    print("      FINAL STRATEGY LEADERBOARD")
    print("=" * 40)
    print(f"{'Rank':<5} {'Strategy':<25} | {'Wins':<10}")
    print("-" * 40)
    for rank, (name, wins) in enumerate(leaderboard, 1):
        print(f"{rank:<5} {name:<25} | {int(wins):<10}")
    print("=" * 40 + "\n")
...
def run_bench():
    ...
    print(f"--- Starting Benchmark (OS Mode: {'Windows' if WINDOWS_MODE else 'Linux'}) ---")
    ...
    ax.set_title(f"Win Matrix (Rows beat Columns)\n{NUM_ROUNDS} Rounds")
...
#     for i in range(NUM_ROUNDS):
#         create_game(num_strats)
#         time.sleep(0.5*sleep_mult_bench)
#         ids = get_waiting_game_ids()
#         if not ids: continue
#         game_id = ids[-1]
#
